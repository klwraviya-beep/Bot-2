...
const config = {
    ...
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '94778430626',
    BOT_FOOTER: '> ρσɯҽɾԃ Ⴆყ ɾαʋιყα ɱԃ🧸',
...
`;
        const aliveMessage = {
            image: { url: "https://files.catbox.moe/rhfqw3.jpeg" },
            caption: `> ι αɱ αʅιʋҽ ɳσɯ 👾\n\n${captionText}`,
            buttons: [
                {
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363424615222831@newsletter',
                newsletterName: 'ρσɯҽɾԃ Ⴆყ ɾαʋιყα ɱԃ',
                serverMessageId: -1
            }
        };
...
`;
    ...
    const menuMessage = {
      image: { url: "https://files.catbox.moe/rhfqw3.jpeg" },
      caption: `*ɾαʋιყα ɱԃ ɱιɳι✨*\n${menuText}`,
      buttons: [
        {
          buttonId: `${config.PREFIX}quick_commands`,
          buttonText: { displayText: 'ɾαʋιყα ɱԃ ɱιɳι ƈɱԃʂ✨' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
              title: 'ɾαʋιყα ɱԃ ɱιɳι ƈɱԃʂ',
              sections: [
                {
                  title: "🌐 ɢᴇɴᴇʀᴀʟ ᴄᴏᴍᴍᴀɴᴅs",
                  highlight_label: 'ɾαʋιყα ɱԃ ɱιɳι',
                  rows: [
  ...
  break;
            ...
            case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363424615222831@newsletter'
                        });
                    }
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    try {
                        const url = `වෙබ් සයිට් එකcode?number=${encodeURIComponent(number)}`;
...
case 'song': {
    ...
    if (!args[0]) {
        return await socket.sendMessage(from, {
            text: 'Please enter YouTube song name or link !!'
        }, { quoted: msg });
    }
    ...
    try {
        ...
        if (!id) {
            // 🔍 Use Mova Nest API for search
            const searchApi = await fetch(`https://movanest.zone.id/v2/ytsearch?query=${encodeURIComponent(q)}`);
            const searchResults = await searchApi.json();

            if (!searchResults?.result?.length) {
                return await socket.sendMessage(from, {
                    text: '🌐 Please enter valid YouTube song name or url.'
                });
            }

            data = searchResults.result[0];
            id = data.url.split("v=")[1];
        } else {
            // If direct link, fetch details
            const searchApi = await fetch(`https://movanest.zone.id/v2/ytsearch?query=https://youtube.com/watch?v=${id}`);
            const searchResults = await searchApi.json();
            if (!searchResults?.result?.length) {
                return await socket.sendMessage(from, {
                    text: '📛 Please enter valid YouTube song name or url.'
        ...
        const { url, title, thumbnail, duration, views, author } = data;

        const caption =
            `*🎧 RAVIYA MD MINI BOT SONG DOWNLOADER*\n\n` +
            `*┏━━━━━━━━━━━━━━━*\n` +
            `*┃ 📌 Title:* ${title || "No info"}\n` +
            `*┃ ⏰ Duration:* ${duration || "No info"}\n` +
            `*┃ 👀 Views:* ${views || "No info"}\n` +
            `*┃ 👤 Author:* ${author || "No info"}\n` +
            `*┃ 📎 URL:* ~${url || "No info"}~\n` +
            `*┗━━━━━━━━━━━━━━━━━━*\n\n` +
        ...
        const templateButtons = [
            {
                buttonId: `${config.PREFIX}mova_mp3 AUDIO ${url}`,
                buttonText: { displayText: '𝙰𝚄𝙳𝙸𝙾 🎧' },
                type: 1,
            },
            {
                buttonId: `${config.PREFIX}mova_mp3 DOCUMENT ${url}`,
                buttonText: { displayText: '𝙳𝙾𝙲𝚄𝙼𝙴𝙽𝚃 📂' },
                type: 1,
            },
            {
                buttonId: `${config.PREFIX}mova_mp4 VIDEO ${url}`,
                buttonText: { displayText: '𝚅𝙸𝙳𝙴𝙾 🎥' },
                type: 1
            }
        ];
...
case 'ai': {
    ...
    const prompt = `ʏᴏᴜʀ name is ɾαʋιყα ɱԃ
...
If asked "Who are you?": Reply "I'm ɾαʋιყα ɱԃ… 👾".  
If asked "Who created you?": Reply "I was created by ɾαʋιყα ɱԃ".  
...
User Message: ${q}
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/rhfqw3.jpeg' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
//===============================
 ...
 // Case: add - Add a member to the group
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/rhfqw3.jpeg' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363424615222831@newsletter',
                    newsletterName: 'ɾαʋιყα ɱԃ ɱιɳι✨',
                    serverMessageId: 143
                }
            }
